...
statistics_df = pd.DataFrame(
    columns=["case", "class_id", "prompt_type", "model", "dsc", "nsd", "running_time"]
)
...
for case_filename in tqdm(cases, desc="Evaluating Cases"):
    ...
    try:
        ...
        for class_idx, box in enumerate(boxes):
            ...
            for prompt_type in ["bbox_3d", "bbox_z_midslice"]:
                ...
                seg_result = PROMPT_MANAGER.add_bbox_interaction(
                    outer_point_one, outer_point_two, include_interaction=True
                )
                ...
                surface_distance = compute_surface_distances(
                    gt_class, seg_result, spacing_mm=spacing
                )
                nsd = compute_surface_dice_at_tolerance(
                    surface_distance, tolerance_mm=2.0
                )
                running_time = time.time() - time_start

                print(
                    f"Class {class_idx + 1}, Prompt Type: {prompt_type}, DSC: {dsc:.4f}, NSD: {nsd:.4f}, Running Time: {running_time:.2f} seconds"
                )
        ...
        print(
            f"Average DSC per prompt type: {statistics_df.groupby('prompt_type')['dsc'].mean()}"
        )
        print(
            f"Average NSD per prompt type: {statistics_df.groupby('prompt_type')['nsd'].mean()}"
        )
        print(
            f"Average Running Time per prompt type: {statistics_df.groupby('prompt_type')['running_time'].mean()}"
        )
